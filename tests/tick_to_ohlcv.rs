...
fn test_tick_to_ohlcv_produces_bars() {
    ...
    if let Some(bar) = agg.flush() { bars.push(bar); }
    assert!(!bars.is_empty());
    for bar in &bars { bar.validate().unwrap(); }
}
...
fn test_tick_to_ohlcv_high_low_correct() {
    ...
    let prices_and_ts = [(dec!(200), 0i64), (dec!(210), 1_000_000_000), (dec!(195), 2_000_000_000)];
    for (p, ts) in prices_and_ts {
        agg.push_tick(&Tick::new(sym.clone(), Price::new(p).unwrap(), Quantity::new(dec!(1)).unwrap(), Side::Ask, NanoTimestamp(ts))).unwrap();
    }
    // Trigger completion with a tick in the next minute
    let result = agg.push_tick(&Tick::new(sym.clone(), Price::new(dec!(205)).unwrap(), Quantity::new(dec!(1)).unwrap(), Side::Ask, NanoTimestamp(nanos_per_min))).unwrap();
